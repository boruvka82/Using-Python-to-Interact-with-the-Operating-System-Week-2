...
Correct Excellent! We have to create a DictWriter() object instance to work with, and pass to it the fieldnames parameter defined as a list of keys.

B]Write the fieldnames parameter into the first row using writeheader()

Correct Nice work! The non-optional fieldnames parameter list values should be written to the first row.

C]Open the csv file using with open

Correct Good call! The CSV file has to be open before we can write to it.
